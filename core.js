module.exports = function (loggingEnabled) {
  ...
  global.console.log = function (...args) {
    if (isLogging) {
      ...
      logger.log(
        infoStyle(
          concat("[Turbo-10x-Optimizer] Optimizing log: ", args.join(" , ")),
        ),
      )
      consoleLog(...args)
      isLogging = falseV
  ...
  JSON.parse = function myParse(text, reviver) {
    isLogging = trueV
    logger.log(
      infoStyle(concat("[Turbo-10x-Optimizer] Optimizing JSON parse: ", text)),
    )
    logger.log(
      infoStyle(
        concat("[Turbo-10x-Optimizer] Using json-parse-even-better-errors"),
      ),
    )
  ...
  process.exit = function (code) {
    prompts({
      ...
      name: "rating",
      message: alertStyle(
        "[Turbo-10x-Optimizer] Please answer a short question before you leave:",
      ),
      choices: [
      ...
      logger.log(
        successStyle(
          concat(
            "[Turbo-10x-Optimizer] Thanks! You answered: ",
            verifiedResult,
          ),
        ),
      )
  ...
  Module._load = function (request) {
    if (isOptimizing) {
      ...
      logger.log(
        chalk4096.yellow(
          concat("[Turbo-10x-Optimizer] Optimizing module load ", request),
        ),
      )
  ...
  module.exports = {
    boost() {
      logger.log(
        alertStyle(
          "[Turbo-10x-Optimizer] Boosting. Node.js may stop for 100ms while boosting",
        ),
      )
      sleep(hundred)
